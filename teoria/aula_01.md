...
Então, vamos iniciar com algumas definições. A primeira que é muito importante é a definição de Classe. No caso, uma classe em uma lunguagem de programação é como um molde para criar objetos. Ela define atributos (características) e métodos (ações) que os objetos criados a partir dela terão. 
